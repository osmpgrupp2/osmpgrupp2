...
pmax(List, N, Death) when length(List) > N ->
    Lists = split(List, N),
    CollectPID = self(),
    Mylist = [{spawn_link(fun() -> worker(L, CollectPID, Death) end),L} || L <- Lists],
    Maxes = collect(length(Lists), [], Mylist, Death),
    pmax(Maxes, N, Death);
pmax(List, _, _) ->
    list:max(List). 
...
collect(N, Maxes,Mylist, Death) when length(Maxes) < N ->
    receive 
	{'EXIT', PID, random_death} ->
	    
	    CollectPID2 = self(),
	    {Key, List} = lists:keyfind(PID,1,Mylist),	    
	    
	    Mylistnew = lists:delete({Key, List}, Mylist),
	    Hej = spawn_link(fun()-> worker(List, CollectPID2, Death)end ),
	    
	    
	    collect(N, Maxes, [{Hej,List}|Mylistnew], Death);
	    
	{'EXIT', _PID, normal} ->
	     collect(N, Maxes, Mylist, Death);
	Max -> 
	    collect(N, [Max|Maxes], Mylist, Death) 
    end;

collect(_N, Maxes, _Mylist, _Death) ->
